...
class SearchForm extends React.Component<IProps, IState> {
    ...
    switch (types) {
      case 'cusNo':
        ...
        if (type === 'commodityOut') {
          ...
          obj.keyWord = value;
          dispatch.commodityInventoryCost.getcusNoList(obj);
        } else {
          obj.tentAndOrg = '1';
          dispatch.commodityInventoryCost.getsupplierList(obj);
        }
        break;
      case 'deptId':
        dispatch.commodityInventoryCost.getDepartmentList();
        break;
      default:
        break;
    }
  ...
  render() {
    ...
    const rules = {
      ...
      cusNo: [
        { required: true, message: type === 'commodityOut' ? '请选择客户姓名' : '请选择供应商', trigger: 'change' },
      ],
    ...
    return (
      <div>
        ...
        >
          <Row gutter={12}>
            ...
            <Col span={8}>
              <FormItem label='调整类型'>
                <Input
                  style={{ width: '100%' }}
                  value={type === 'commodityOut' ? '出库成本调整' : '入库成本调整'}
                  disabled
                />
              </FormItem>
            </Col>
          ...
          <Row gutter={12}>
            <Col span={8}>
              <FormItem label={type === 'commodityOut' ? '客户' : '供应商'} prop='cusNo'>
                <Select
                  style={{ width: '100%' }}
                  value={cusNo}
                  filterable={true}
                  ...
                  clearable={true}
                >
                  {type === 'commodityOut' ? cusNos : suppliers}
                </Select>
              </FormItem>
            </Col>
            <Col span={8}>
